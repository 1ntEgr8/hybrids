...
function resolve(config, model, lastModel) {
  ...
  if (model) definitions.set(model, config);
...
function resolveKey(key, config) {
  ...
  if (!config) {
    ...
    const transform = Object.keys(Object.freeze(Model))
      ...
      .map(key => {
        ...
        const { defaultValue, type } = resolveKey(key, config);
...
function getValidationError(errors) {
  ...
  const e = Error(
    `Model validation failed (${keys.join(
      ", ",
    )}) - read the details from 'fields' property`,
  );
    ...
    if (localModel) {
      config.checks.forEach((fn, key) => {
        if (
          ...
          localModel[key] == config.model[key] &&
          keys.indexOf(key) === -1 &&
          (!lastError || !lastError.fields || !lastError.fields[key])
        ) {
          return;
        }
...
function error(model, property) {
  ...
  if (result && property !== undefined) {
    return result.fields && result.fields[property];
  }
...
function submit(draft) {
  ...
  if (!config || !draftMap.has(config)) {
    throw TypeError("Provided model instance is not a draft");
  }
...
function valueWithValidation(
  ...
  if (validate instanceof RegExp) {
    ...
    fn = (...args) => validate(...args) || errorMessage;
  } else {
    throw TypeError(
